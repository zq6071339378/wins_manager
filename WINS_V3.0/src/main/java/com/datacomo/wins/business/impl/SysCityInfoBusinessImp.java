 ...
 *
 */
@Repository("sysCityInfoBusiness")
public class SysCityInfoBusinessImp implements SysCityInfoBusiness{

	private static Logger logger = Logger.getLogger(SysCityInfoBusinessImp.class.getName());
	@Autowired
	...
	public Map<String, Object> getInfo(int visitId, int id) {
		logger.info("getInfo method start");
	...
	public List<Map<String, Object>> findByCondition(int visitId,
			Map<String, Object> condition) {
		logger.info(SysCityInfoBusinessImp.class.getName()+" findByCondition start");
		if(logger.isDebugEnabled()){
			logger.debug("visitId:"+visitId);
		...
		try {
			result = sysCityInfoMapper.findByCondition(condition);
		} catch (Exception e) {
			e.printStackTrace();
			logger.error(e);
			result=null;
		...
		logger.info(SysCityInfoBusinessImp.class.getName()+" findByCondition end");
