					...
					<div class="col-md-12 row_left">
						<div class="col-md-11 row_left">
							<div class=" btn-group col-md-5">
								<a href="#policy/policyList?policyType=0"
									onclick="load_page('policy/policyList?policyType=0');"
									class="btn btn-default"
									<c:if test="${policyType eq 0 }"> style="background-color:#EBEBEB;"</c:if>><%=Config.message.get("POLICY_MANAGEMENT_ALL")%></a>
								<a href="#policy/policyList?policyType=1"
									onclick="load_page('policy/policyList?policyType=1');"
									class="btn btn-default"
									<c:if test="${policyType eq 1 }"> style="background-color:#EBEBEB;"</c:if>><%=Config.message.get("POLICY_MANAGEMENT_RUNNING")%></a>
								<a href="#policy/policyList?policyType=2"
									onclick="load_page('policy/policyList?policyType=2');"
									class="btn btn-default"
									<c:if test="${policyType eq 2 }"> style="background-color:#EBEBEB;"</c:if>><%=Config.message.get("POLICY_MANAGEMENT_TORUN")%></a>
								<a href="#policy/policyList?policyType=3"
									onclick="load_page('policy/policyList?policyType=3');"
									class="btn btn-default"
									<c:if test="${policyType eq 3 }"> style="background-color:#EBEBEB;"</c:if>><%=Config.message.get("POLICY_MANAGEMENT_AUDIT")%></a>
								<a href="#policy/policyList?policyType=4"
									onclick="load_page('policy/policyList?policyType=4');"
									class="btn btn-default"
									<c:if test="${policyType eq 4 }"> style="background-color:#EBEBEB;"</c:if>><%=Config.message.get("POLICY_MANAGEMENT_STOPEED")%></a>
							</div>
							<!-- 按广告主搜索策略 -->
							...
							<div class="col-md-5  login-box" style="width:40%">
								<form action="index.html#policy/policyList?policyType=${policyType}" method="post" id="searchPolicyForm">
									<div class="form-group col-md-8 search_icon">
										...
										<input type="search" name="searchPolicyName" style="padding-left:5px;" id="exampleInputSearch" class="form-control" placeholder="<%=Config.message.get("POLICYVIEW_PLEASEINPUTPOLICYNAME")%>">
									</div>
						...
						<c:if test="${globaltemp eq 1}">
							<div class="col-md-1 login-box">
								<form>
									<div class="from-group col-md-12" style="margin-right:30px;">
										...
										<table
											...
											<thead>
												<tr>
													...
													<th><%=Config.message.get("POLICY_MANAGEMENT_AREA")%></th>
													<th><%=Config.message.get("POLICY_MANAGEMENT_TARGETUSERS")%></th>
											...
											<tbody>
												<c:forEach items="${result.POLICYLIST}" var="policyBean" varStatus="i" step="1">
													<tr>
														...
														<td>${policyBean.policyId }</td>
														<td>${policyBean.policyName }</td>
														<td>${policyBean.createName }</td>
														<td>
															<c:if test="${!empty policyBean.provinceId}">
																<c:if test="${policyBean.cityId eq 0}">
																	${policyBean.provinceName}
																</c:if>
																<c:if test="${policyBean.cityId ne 0}">
																	${policyBean.cityName}
																</c:if>
															</c:if>
															<c:if test="${empty policyBean.provinceId}">
																<%=Config.message.get("ALL_AREA")%>
															</c:if>
														</td>
														<td>${policyBean.groupUses}</td>
														...
														<td><c:if test="${policyBean.checkStatus eq 0}"><%=Config.message.get("POLICY_MANAGEMENT_TOAUDIT")%></c:if>
															<c:if test="${policyBean.checkStatus eq 1}"><%=Config.message.get("POLICY_MANAGEMETN_AUDITED")%></c:if></td>
								...
								<div class="row">
									<div class="dataTables_footer clearfix">
										<jsp:include page="../layout/pagination.jsp"><jsp:param
												value="policy/policyList?policyType=${policyType }"
          ...
          /*    function editeOne();{
				...
				function updateStatus(_obj){
					...
					$.ajax({
						   ...
						   success: function(data){
							   if(data.result>0){
								   load_page('policy/policyList?policyType='+_policyType);
							   }else{
								   alert("Failed!");
							   }
						   }
				...
				//搜索
				   function searchPolicy(){
					   ...
					   load_page('policy/policyList?policyType='+_policyType+'&searchType='+_searcheType+'&searchPolicyName='+decodeURIComponent(_searchPolicyName));
				   }
